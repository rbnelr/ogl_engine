...
namespace time {
    ...
    DECLD u64					gpu_zerotime; // earliest measurement of gpu timestamp, (opengl context needs to exist to allow measuremnt)
    DECLD f64					gpu_process_begin; // calculated process begin gpu timestamp
    
    DECL void measure_gpu_zerotime () {
        u64 cpu = QPC::get_time();
        {
            s64 temp;
            glGetInteger64v(GL_TIMESTAMP, &temp);
            gpu_zerotime = (u64)temp;
        }
        
        u64 cpu_diff = cpu -qpc_process_begin;
        f64 cpu_sec = (f64)cpu_diff * QPC::inv_freq;
        
        f64 gpu_abs_sec = gpu_zerotime * nano<f64>(1);
        f64 gpu_process_begin_sec = gpu_abs_sec -cpu_sec;
...
namespace time {
    ...
    DECLD u64			qpc_prev_frame_end;
    DECLD u64			qpc_this_frame_work_end;
    ...
    DECLD u64			qpc_frame_period;
    
    DECL void process_begin () {
        qpc_process_begin = QPC::get_time();
        
        profile::record_zerotime(qpc_process_begin);
        profile::init();
    }
    
    DECL void cpu_work_done () {
        qpc_this_frame_work_end = QPC::get_time();
    }
    ...
    DECL void frame_end () {
        ...
        {
            f64 diff =				(f64)qpc_frame_period;
            
            dt64 =					diff * QPC::inv_freq;
            
            f64 msecs =				dt64 * 1000.0;
            f64 fps =				QPC::freq / diff;
            
            f64 cpu_work_msec =		((f64)qpc_this_frame_work_diff * QPC::inv_freq) * 1000.0;
            f64 cpu_work_percent =	(cpu_work_msec / msecs) * 100.0;
        ...
        qpc_prev_frame_end =	this_frame_end;
    }
    
    DECL void application_close_timing () {
        profile::finish_file();
    }
}
...
namespace platform {
    ...
    DECL void setup_renderer_context () {
        ...
        {
        PROFILE_BEGIN(THR_ENGINE, "dummy window CreateWindowExA()");
        ...
        PROFILE_END(THR_ENGINE, "dummy window CreateWindowExA()");
            
        PROFILE_BEGIN(THR_ENGINE, "dummy window GetDC()");
            HDC dummy_hdc;
            {
                dummy_hdc= GetDC(dummy_hwnd);
                assert(dummy_hdc != NULL);
            }
        PROFILE_END(THR_ENGINE, "dummy window GetDC()");
            
            PIXELFORMATDESCRIPTOR pfd = {}; // Initialize to zero
            {
                PROFILE_SCOPED(THR_ENGINE, "dummy window ChoosePixelFormat() & SetPixelFormat()");
            ...
            HGLRC dummy_hglrc;
            {
                PROFILE_SCOPED(THR_ENGINE, "dummy window wglCreateContext()");
            ...
            {
                PROFILE_SCOPED(THR_ENGINE, "dummy window wglMakeCurrent()");
            ...
            { // WGL_ARB_pixel_format; requires WGL_ARB_extensions_string
                PROFILE_SCOPED(THR_ENGINE, "dummy window wglChoosePixelFormatARB()");
            ...
            {
                PROFILE_SCOPED(THR_ENGINE, "real window SetPixelFormat()");
            ...
            {
                PROFILE_SCOPED(THR_ENGINE, "real window wglCreateContextAttribsARB()");
            ...
            if (hGLContext != NULL) {
                {
                    PROFILE_SCOPED(THR_ENGINE, "dummy window destroy");
                    {
                ...
                {
                    PROFILE_SCOPED(THR_ENGINE, "real window wglMakeCurrent()");
        ...
        {
            PROFILE_SCOPED(THR_ENGINE, "OGL functions load");
    ...
    DECLA DWORD WINAPI engine_main (LPVOID lpParam) {
        ...
        {
            u64 now = time::QPC::get_time();
            
            profile::process_chunk(time::qpc_process_begin, now, "init");
            time::measure_gpu_zerotime();
            profile::init_gl();
            
            time::qpc_prev_frame_end = now;
        }
        ...
        
        {
            ...
            profile::process_chunk(time::qpc_prev_frame_end, now, "gl_init");
        ...
        for (;; ++frame_number) {
            ...
            #if 0
            {
                PROFILE_SCOPED(THR_ENGINE, glFinish);
                glFinish();
            }
            #endif
            
            {
                PROFILE_SCOPED(THR_ENGINE, "WAIT:SwapBuffers");
                SwapBuffers(hDC);
            }
                ...
                switch (rawInput.header.dwType) {
                    case RIM_TYPEMOUSE: {
                        ...
                        #endif
                        {
                            ...
                            static constexpr WIN_H_USHORT buttonFlagsArr[_NUM_BUTTONS * 2] = {
                            ...
                            for (button_e button=_FIRST_BUTTON; button<_NUM_BUTTONS; ++button) {
                                auto indxDown =	(button * 2) +0;
                                auto indxUp =	(button * 2) +1;
