...
namespace print_n {
    ...
    DECL void init () {
        ...
        
        {
            ...
            cstr error_filename = print_working_stk("logs/%_%_%.%.%-%.%.%.error.log\\0", pc_name, exe_filename,
                loc.wYear, loc.wMonth, loc.wDay,
                loc.wHour, loc.wMinute, loc.wSecond ).str;
            
            cstr full_filename = print_working_stk("logs/%_%_%.%.%-%.%.%.full.log\\0", pc_name, exe_filename,
                loc.wYear, loc.wMonth, loc.wDay,
                loc.wHour, loc.wMinute, loc.wSecond ).str;
